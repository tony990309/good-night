FactoryBot.define do
  factory :user do
    sequence :name do |n|
      "User #{n}"
    end

    trait :create_sleeping_duration do
      name { 'sleeping_user' }
      after(:create) do |user|
        create(:sleep_clock_in, user: user)
        create(:sleeping_duration, user: user)
      end
    end

    trait :create_awake_duration do
      name { 'awake_user' }
      after(:create) do |user|
        create(:sleep_clock_in, user: user)
        create(:wake_clock_in, user: user)
        create(:awake_duration, user: user)
      end
    end

    trait :create_two_following_users do
      after(:create) do |user|
        user.following << create(:sleeping_user)
        user.following << create(:awake_user)
      end
    end

    factory :sleeping_user, traits: [:create_sleeping_duration]
    factory :awake_user, traits: [:create_awake_duration]
    factory :two_following_user, traits: [:create_two_following_users]
  end
end
